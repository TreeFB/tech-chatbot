...
const MeetingUploadForm = ({ selectedOrganisations, setSelectedOrganisations, includeMeetingTime }) => {
  ...
  // helper to normalise API objects (Id/Name -> id/name)
  ...
  const loadOrganisationOptions = async (searchString) => {
    if (searchString.length > 2) {
        ...
        if (response.ok) {
            var organisationOptions = await response.json();
            return normalizeArray(organisationOptions);
        }
    }
    return [];
  };
  ...
  const BotMessageButton = ({ title, icon }) => {
    ...
    return  <button className="button_icon submit" onClick={handleClick} disabled={formSubmitted||organisation.id==0||projects.length==0||requestingTeam==""||writerTeam==""||techSector==""||writingDueDate==""||(includeMeetingTime&&meetingTime=="")}>
